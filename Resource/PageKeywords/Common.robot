...
Verify elements is visible and displayed
    ...
    [Arguments]    ${locator}
    SeleniumLibrary.wait until element is enabled    ${locator}    timeout=60
...
Set Dropdown3
    ...
    IF    not ${bool}
        ...
        #Click Item  //div[text()='${locator_div}']
         SeleniumLibrary.Click Element  //div[@aria-disabled='false' and text()='${locator_div}']
    END
...
Set Dropdown5
    ...
    #wait until element is visible    ${dropdown}
    #Wait Until Keyword Succeeds    ${TimeOut}      ${Start}      click element      ${dropdown}
    ${bool}=    Execute Javascript      var a = document.querySelectorAll('input[type=radio]');for(var i=0;i<a.length;i++){if(a[i].checked==true && a[i].nextSibling.innerHTML.trim() === '${Month}') return true; else return false;}
    Log To Console  ------------------------
    Log To Console  ${bool}
    Log To Console  ------------------------
    IF    not ${bool}
